  ...
  class Pos extends CI_Controller{
    ...
    public function index(){
      ...
      $data['tableData'] = $this->mrestaurant->getRetaurantTableDataActive();


      $this->load->view('includes/header_db');
      $this->load->view('superadmin/navigation');
      $this->load->view('pos/home',$data);
    ...
    public function generatePosTable(){
      ...
      $texData = $this->mcrud->getDataByIdOrder('taxservices','Restaurant','name','modifiedDate');
      ...
      $this->load->view('pos/posTable',$main);
    }
   ...
   public function generatePosInvoice(){
    if ($this->session->userdata('passth')) {
      ...
      $this->load->view('includes/header_db');
      $this->load->view('superadmin/navigation');
      $this->load->view('pos/posinvoice',$data);
      $this->load->view('includes/footer_inc_form');  
    ...
    public function paidBooking(){
      $inNo = $_POST["invno"];
      $result2 = $this->mcrud->updateDataByForm('pos_invoices',array("status"=>"Paid","invfrom"=>"Restaurant"),array("invno"=>$inNo));
    ...
    public function linkWithBooking(){
      $inNo = $_POST["invno"];
      $bookNo = $_POST["booking_no"];
      $result = $this->mcrud->updateDataByForm('pos_invoices',array("status"=>"Linked","booking_no"=>$bookNo,"invfrom"=>"Restaurant"),array("invno"=>$inNo));
      if ($result) {
        echo 1;
      }else{
        echo 0;
