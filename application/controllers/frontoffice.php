  ...
  class Frontoffice extends CI_Controller{
    ...
    public function addNewBooking(){
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $updatedadb = $this->mcrud->updateDataByForm('advance_booking_room',$adbData,array('id' => $this->input->post('adbLinked')));
    ...
    public function occupiedRoom(){
      ...
      $roomtypeForFilter = $data['roomInfo'][0]->rtyId;

      $data["availableRooms"] = $this->mbooking->getRoomsOfSameTyeSameStatus('Available',$roomtypeForFilter);
    ...
    public function extendBooking(){
      $new_chkout_date = $this->input->post("chkout_date");
      if ($new_chkout_date != "") {
        $newCheckoutDate = $this->convertDateFromPicker($new_chkout_date);

        $roomData = array(
          'chkout_date'=>$newCheckoutDate,
          );
        $updatedCheckout = $this->mcrud->updateDataByForm('booking_room',$roomData,array('booking_no' => $this->input->post('bookingno')));
      }
      $selectedRoomId = $this->input->post("newRoomNo");
      if ($selectedRoomId != "") {
        $roomData2 = array(
          'availibility'=>'Occupied',
          );        
        $updateNewRoom = $this->mcrud->updateDataByForm('room_master',$roomData2,array('roomno' => $this->input->post('newRoomNo')));

        $roomData3 = array(
          'availibility'=>'Housekeeping',
          );        
      ...
      if ($updatedCheckout || $updateNewRoom || $updateOldRoom) {
        $this->session->set_userdata('success', ' Booking Changes done succesfully!');
